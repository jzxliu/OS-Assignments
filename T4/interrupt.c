...
interrupt_handler(int sig, siginfo_t * sip, void *contextVP)
{
	...
	if (loud) {
		 ...
		 * file descriptor. 
		 *
		 * You can experiment with the two methods by defining either 
		 * USE_PRINTF_IN_SIGNAL_HANDLER or USE_WRITE_IN_SIGNAL_HANDLER
		 * in the Makefile.
		 */
#ifdef USE_PRINTF_IN_SIGNAL_HANDLER
		printf("%s: context at %10p, time diff = %ld us\n",
		       __FUNCTION__, context,
		       (diff.tv_sec * NSEC_PER_SEC + diff.tv_nsec)/1000);
#endif
#ifdef USE_WRITE_IN_SIGNAL_HANDLER
		char msgbuf[80];
		snprintf(msgbuf, 80, 
...
#endif /* USE_WRITE_IN_SIGNAL_HANDLER */
	}
